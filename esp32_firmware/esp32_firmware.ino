...
float STEPS_PER_PIX    = 3.5f;   // 조이스틱 감도: 숫자를 낮추면 모터가 적게 돌고, 높이면 많이 돕니다. (2.0 ~ 6.0 추천)

// ── 기어비: 16T(모터) → 80T(출력) = 1:5 ─────────────────────────
// 모터가 5바퀴 = 출력축 1바퀴. 큰 힘과 속도가 필요하므로 최고속도 대폭 상향.
// 가속도: 0 → MAX 도달에 약 375ms (1초 미만 부드러운 가속)
float MAX_SPEED_LIMIT  = 3000.0f; // 모터 최고 속도 Hz (출력축 기준 600Hz, 충분한 속도)
float ACCELERATION_RATE= 8.0f;    // 가속도 Hz/ms: 3000÷8=375ms 만에 최고속 도달 (1초 미만)
