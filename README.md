...
> 
> **🏆 v1.3.0 업데이트 내역:**
> - **하드웨어 맞춤형 물리 각도 실측 적용**: M1(Pan) ±180도, M2(Tilt) +50도/-40도 실측 스케일 기반의 완벽한 리밋 제어
> - **다이나믹 2D 충돌 방지 안전 구역(Safe Zone)**: 카메라가 하단(-25도 이하)으로 향할 때 자동으로 수평(M1) 회전 반경을 축소(±80도)하여 구조물 충돌 원천 차단
> - **자동 감속 제동 시스템 (Soft Braking)**: 한계 지점 도달 전(15~20도 전방)부터 조이스틱 신호를 강제로 서서히 줄여, 고속 이동 시 밀림(Overshoot) 현상 없이 완벽하고 부드러운 정지 구현
> - **실시간 터미널(TUI) 경고창**: 하드웨어 리밋 차단 원인(예: 2D Safe Zone 등) 실시간 표시 기능 추가
> - **웹 UI 전체화면 지원**: 모바일/PC 모두 앱처럼 꽉 찬 화면으로 사용 가능한 Fullscreen 버튼 탑재
<br/>
